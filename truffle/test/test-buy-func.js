    ...
    describe("test buy func", async () => {
        describe("test with valid parameters", async () => {
            it("try to buy a product", async () => {
                ...
                await cryptoMarketplace.buy(productID, 1, {from: owner, value: 1 * 10 ** 18});
            ...
            it("try to buy two times a product", async () => {
                ...
                await cryptoMarketplace.buy(productID1, 2, {from: owner, value: 4 * 10 ** 18});
                await cryptoMarketplace.buy(productID1, 1, {from: owner, value: 2 * 10 ** 18});
                let product = await cryptoMarketplace.getProduct.call(productID);
            ...
            it("try to buy five times a product", async () => {
                ...
                await cryptoMarketplace.buy(productID1, 1, {from: owner, value: 2 * 10 ** 18});
                await cryptoMarketplace.buy(productID1, 2, {from: owner, value: 4 * 10 ** 18});
                await cryptoMarketplace.buy(productID1, 5, {from: owner, value: 10 * 10 ** 18});
                await cryptoMarketplace.buy(productID1, 1, {from: owner, value: 2 * 10 ** 18});
                await cryptoMarketplace.buy(productID1, 3, {from: owner, value: 6 * 10 ** 18});
                let product = await cryptoMarketplace.getProduct.call(productID);
            ...
            it("try to buy a product with more money then needed", async () => {
                let productID = await cryptoMarketplace.newProduct("Domati", 1, 2);
                await cryptoMarketplace.buy(productID, 1, {from: owner, value: 3 * 10 ** 18});
                let product = await cryptoMarketplace.getProduct.call(productID);
            ...
            it("try to add and buy multiply products and check first", async () => {
                ...
                await cryptoMarketplace.buy(productID3, 1, {from: owner, value: 2 * 10 ** 18});
                await cryptoMarketplace.buy(productID2, 1, {from: owner, value: 10 * 10 ** 18});
                await cryptoMarketplace.buy(productID1, 1, {from: owner, value: 1 * 10 ** 18});
        ...
        describe("test with invalid parameters", async () => {
            it("try to buy zero products", async () => {
                let productID1 = await cryptoMarketplace.newProduct("Bob", 2, 10);
                expect(await cryptoMarketplace.buy(productID1, 0, {from: owner, value: 1 * 10 ** 18})).to.throw();
            })
    
            it("try to buy zero products", async () => {
                let productID1 = await cryptoMarketplace.newProduct("Bob", 2, 10);
                expect(await cryptoMarketplace.buy(productID1, -5, {from: owner, value: 1 * 10 ** 18})).to.throw();
            })
    
            it("try to buy zero products", async () => {
                let productID1 = await cryptoMarketplace.newProduct("Bob", 2, 10);
                expect(await cryptoMarketplace.buy(productID1, 20, {from: owner, value: 40 * 10 ** 18})).to.throw();
            })
    
            it("try to buy zero products", async () => {
                let productID1 = await cryptoMarketplace.newProduct("Bob", 2, 10);
                expect(await cryptoMarketplace.buy(productID1, 54545545454545, {from: owner, value: 40 * 10 ** 18})).to.throw();
            })
    
            it("try to buy products with less money", async () => {
                let productID1 = await cryptoMarketplace.newProduct("Bob", 2, 10);
                expect(await cryptoMarketplace.buy(productID1, 2, {from: owner, value: 2 * 10 ** 18})).to.throw();
            })
    
            it("try to buy with wrong id", async () => {
                let productID1 = await cryptoMarketplace.newProduct("Bob", 2, 10);
                expect(await cryptoMarketplace.buy(keccak256("Bob", 2, 10), 2, {from: owner, value: 4 * 10 ** 18})).to.throw();
            })
    
            it("try to buy with quantity pesho", async () => {
                let productID1 = await cryptoMarketplace.newProduct("Bob", 2, 10);
                expect(await cryptoMarketplace.buy(productID1, "pehso", {from: owner, value: 2 * 10 ** 18})).to.throw();
            })
            ...
            it("try to buy with one param", async () => {
                let productID1 = await cryptoMarketplace.newProduct("Bob", 2, 10);
                expect(await cryptoMarketplace.buy(productID1, {from: owner, value: 2 * 10 ** 18})).to.throw();
            })
    
            it("try to buy with three params", async () => {
                let productID1 = await cryptoMarketplace.newProduct("Bob", 2, 10);
                expect(await cryptoMarketplace.buy(productID1, 1, 10, {from: owner, value: 2 * 10 ** 18})).to.throw();
            })
